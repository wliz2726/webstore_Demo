...
class ControllerCatalogProduct extends Controller {
    ...
    public function getColorSwatchAdm() {
        $this->load->model('catalog/product');
        $this->load->model('catalog/option');
        $this->load->model('tool/image');

        $colorswatch_images = $this->model_catalog_product->getColorSwImages($this->request->get['colorsw_id']);

        $data['colorswatch_images'] = array();

        foreach ($colorswatch_images as $colorswatch_image) {
            if (is_file(DIR_IMAGE . $colorswatch_image['image'])) {
                $image = $colorswatch_image['image'];
                $thumb = $colorswatch_image['image'];
            } else {
                $image = '';
                $thumb = 'no_image.png';
            }
            ...
            $data['colorswatch_images'][] = array(
                ...
                'sort_order' => $colorswatch_image['sort_order']
            );
        }
    ...
    public function setColorSwatchAdm() {

        $this->load->model('catalog/product');
        $this->load->model('catalog/option');
        $this->load->model('tool/image');

        $colorswatch_images = $this->model_catalog_product->setColorSwImages($this->request->post);
	...
	protected function getForm() {
        ...
        // ColorSwatch Images
            ...
            $colorswatch = array();
            $colorswatch = $configArr['colorswatch'];
			$xytx = $colorswatch['featured_tml'];
			$xytx = (((($xytx / 6) * 4) / 10) - 12);
			$data['wertyx'] = $xytx;

			$bkslmt = count($this->model_catalog_product->getColorSwLimit());
			if ($bkslmt >= $xytx) {

				$data['colorswatch_frlmt'] = "0";
				$varvalcolorsw = $this->model_catalog_product->getColorSwLimit();

				foreach ($varvalcolorsw as $value) {

					if ($this->request->get['product_id'] == $value['product_id']) {
						$data['colorswatch_frlmt'] = "1";
					}
   		...
   		$data['colorswatch_productid'] = $this->request->get['product_id'];

        if (isset($this->request->post['colorswatch_image'])) {
            $colorswatch_images = $this->request->post['colorswatch_image'];
        } elseif (isset($this->request->get['product_id'])) {
            $colorswatch_images = $this->model_catalog_product->getColorSwImages($this->request->get['product_id']);
        } else {
            $colorswatch_images = array();
        }
		...
		if (!$this->user->hasPermission('modify', 'extension/module/colorswatch')) {
			$data['colorswPermission'] = "nomodify";
		}
		else {
			$data['colorswPermission'] = "modify";
		}
